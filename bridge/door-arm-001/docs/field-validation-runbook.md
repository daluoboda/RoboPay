...
cd RoboPay
git checkout feat/door-arm-001-tier1
```
...
# List available skills (should show door variant)
python3 -m bridge.door-arm-001 --discover
```

Expected: skill `door-arm-001` listed with price 0.1 USDC on Base Sepolia.
...
```bash
# Run full test suite
pytest bridge/door-arm-001/tests/ -v

# Or trigger CI workflow (requires maintainer approve on fork PR)
gh run watch --workflow=door-arm-001-ci.yml
```
...
|----------|----------|----------|
| `validation-report.md` | `bridge/door-arm-001/docs/` | All 7 criteria covered |
| `x402-evidence.json` | Root | Real tx hashes, block numbers |
| `task-traceability.md` | `bridge/door-arm-001/docs/` | Test-to-criterion mapping |
| `field-validation-runbook.md` | `bridge/door-arm-001/docs/` | This runbook |
| `test_safe_stop.py` | `bridge/door-arm-001/tests/` | Criterion #5 verification |
| `test_payment_gate.py` | `bridge/door-arm-001/tests/` | Criterion #1, #4 verification |
| `settle.png` | `bridge/door-arm-001/docs/evidence/` | Terminal payer=0xf274 |
| `door-arm-demo.mp4` | `bridge/door-arm-001/docs/evidence/` | Video of successful action |
| `evidence-manifest.yaml` | `bridge/door-arm-001/docs/evidence/` | SHA-256 of all evidence files |
