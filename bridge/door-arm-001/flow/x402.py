"""x402 payment verification for door-arm-001 (D7 payment boundary).
...
class X402Verifier:
    ...
    def verify(self, payment: dict, challenge: Optional[X402Challenge] = None) -> dict:
        challenge = challenge or X402Challenge("open_door")
        if not payment:
...
def verify_payment(payment: dict | None) -> dict:
    """Verify a payment receipt against the open_door x402 challenge.
