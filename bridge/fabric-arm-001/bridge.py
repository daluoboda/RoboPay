"""Zenoh bridge for paid fabric-arm-001 ``pick_object`` actions.
...
LOGGER = logging.getLogger("robopay.fabric")

ACTION_TOPIC = "robot/tunnel/action"
RESULT_TOPIC = "robot/tunnel/result"
METRICS_TOPIC = "robot/fabric-arm-001/metrics"

ROBOT_ID = "fabric-arm-001"
ALLOWED_ACTIONS = {"pick_object"}
PROFILE_ID = "fabric.fabric-arm-001.mujoco-sim.v1"
...
class FabricZenohBridge:
    ...
    def __init__(self, settings: BridgeSettings | None = None):
        try:
            ...
            raise RuntimeError("Install eclipse-zenoh to run the fabric bridge.") from error
    ...
    def spin(self) -> None:  # pragma: no cover - integration entry point
        LOGGER.info(
            "fabric-arm-001 bridge %s listening on %s; publishing results on %s",
...
def main() -> None:  # pragma: no cover - integration entry point
    """Run the fabric-arm-001 bridge as a standalone Zenoh worker."""
