...
on a single Zenoh network, and asserts:
  ...
  * a paid ``pick_object`` whose real MuJoCo run SUCCEEDS (object="cube")
  ...
  * a paid ``pick_object`` whose real MuJoCo run FAILS (object="unreachable")
    ...
    def _run_once(self, object_name: str):
        ...
        try:
            ...
            status, headers, _ = http_post(
                action_url, {"action": "pick_object", "params": {"object": object_name}}
            )
