"""Exercise fabric-arm-001's x402 payment gate through the real Go Tunnel binary.

Covers every point of PR #70's CHANGES_REQUESTED:
...
# bridge/fabric-arm-001/tests -> bridge/fabric-arm-001 -> bridge -> repo root
PACKAGE_ROOT = Path(__file__).resolve().parents[1]
ROOT = PACKAGE_ROOT.parents[2]
SKILL_CATALOG = (
    ROOT
    / "registry/vendors/laok/fabric-arm-001"
    / "laok.fabric-arm-001.pick.v1/skill-catalog.json"
)
BRIDGE_PYTHONPATH = str(PACKAGE_ROOT)
ROBOT_ID = "fabric_arm_001_payment_gate"
ZENOH_TEST_PORT = int(os.environ.get("FABRIC_PAYMENT_GATE_ZENOH_PORT", "7447"))
PRICE = "0.10"
ALLOWED_ACTIONS = "pick_object,stop"
...
class SimulatorSide:
    ...
    def _on_action(self, sample) -> None:
        ...
        res = self.executor.execute(skill_id or "pick_object", params)
...
class FabricPaymentGateTests(unittest.TestCase):
    ...
    def test_unpaid_malformed_and_facilitator_rejected_requests_fail_closed(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="fabric_payment_gate_") as temp_dir:
                ...
                unpaid_status, unpaid_headers, _ = http_post(
                    action_url, {"action": "pick_object", "robot_id": ROBOT_ID}
                )
                ...
                malformed_status, _, _ = http_post(
                    action_url,
                    {"action": "pick_object", "params": "not-an-object"},
                )
                ...
                tampered_id = f"fabric-tampered-{uuid.uuid4().hex}"
                rejected_status, _, _ = self._paid_post(
                    action_url, unpaid_headers, tampered_id, {"object": "cube"}
                )
                ...
                print("[FABRIC DISCOVERY] robot + skills + price: OK")
                print("[FABRIC PAYMENT GATE] unpaid/malformed/isValid:false -> HTTP 402, zero ActionEvents")
        finally:
    ...
    def test_paid_action_publishes_and_settles(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="fabric_paid_") as temp_dir:
                temp_dir = Path(temp_dir)
                config_path, _ = self._write_configs(temp_dir)
                tunnel = self._start_tunnel(
                    tunnel_binary, config_path, temp_dir, proxy, facilitator
                ...
                paid_id = f"fabric-paid-{uuid.uuid4().hex}"
                paid_status, _, _ = self._paid_post(
                    action_url, unpaid_headers, paid_id, {"object": "cube"}
                )
                ...
                print("[FABRIC PAID] verified payment -> ActionEvent -> correlated result -> settle: OK")
        finally:
            self._teardown(proxy, facilitator, facilitator_thread, observer, simulator, tunnel)
    ...
    def test_failed_execution_does_not_settle(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="fabric_fail_") as temp_dir:
                temp_dir = Path(temp_dir)
                config_path, _ = self._write_configs(temp_dir)
                tunnel = self._start_tunnel(
                    tunnel_binary, config_path, temp_dir, proxy, facilitator
                ...
                failed_id = f"fabric-fail-{uuid.uuid4().hex}"
                paid_status, _, _ = self._paid_post(
                    action_url, unpaid_headers, failed_id, {"object": "unreachable"}
                )
                ...
                print("[FABRIC FAILURE] failed execution -> no settlement: OK")
        finally:
            self._teardown(proxy, facilitator, facilitator_thread, observer, simulator, tunnel)
    ...
    def test_timeout_does_not_settle(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="fabric_timeout_") as temp_dir:
                temp_dir = Path(temp_dir)
                config_path, _ = self._write_configs(temp_dir)
                tunnel = self._start_tunnel(
                    tunnel_binary, config_path, temp_dir, proxy, facilitator
                ...
                timeout_id = f"fabric-timeout-{uuid.uuid4().hex}"
                paid_status, _, _ = self._paid_post(
                    action_url, unpaid_headers, timeout_id, {"object": "cube"}
                )
                self.assertEqual(paid_status, 202)
                ...
                print("[FABRIC TIMEOUT] no simulator result -> timeout -> no settlement: OK")
        finally:
            self._teardown(proxy, facilitator, facilitator_thread, observer, simulator, tunnel)
