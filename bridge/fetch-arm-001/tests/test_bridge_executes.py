...
+ facilitator) and the actual ``bridge.FetchZenohBridge`` (real MuJoCo physics)
on a single Zenoh network, and asserts:

  * a paid ``fetch_object`` whose real MuJoCo run SUCCEEDS (object="fetchable")
  ...
  * a paid ``fetch_object`` whose real MuJoCo run FAILS (object="unreachable")
    ...
    def _run_once(self, object_name: str):
        ...
        try:
            ...
            status, headers, _ = http_post(
                action_url, {"action": "fetch_object", "params": {"object": object_name}}
            )
