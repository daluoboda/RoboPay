...
ROBOT_ID = "fetch-arm-001-no-settle"
...
class NoSettlementTests(unittest.TestCase):
    def _start(self, with_simulator: bool = True):
        ...
        simulator = InjectedFetchSimulator(robot_id=ROBOT_ID) if with_simulator else None
        tunnel = None
        proxy.start()
        try:
            tunnel = launch_tunnel(
                ...
                Path(tempfile.mkdtemp(prefix="fabric_no_settle_")),
            )
    ...
    def _challenge(self, action_url: str) -> dict:
        status, headers, _ = http_post(action_url, {"action": "fetch_object", "params": {"object": "fetchable"}})
