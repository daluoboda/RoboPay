"""Zenoh bridge for paid push-arm-001 ``push_box`` actions.
...
actions, runs the real MuJoCo contact-push physics for ``push_box``, and
publishes the terminal result echoing the correlation tuple the Tunnel issued
...
LOGGER = logging.getLogger("robopay.push")

ACTION_TOPIC = "robot/tunnel/action"
RESULT_TOPIC = "robot/tunnel/result"
METRICS_TOPIC = "robot/push-arm-001/metrics"

ROBOT_ID = "push-arm-001"
ALLOWED_ACTIONS = {"push_box"}
PROFILE_ID = "push.push-arm-001.mujoco-sim.v1"

KNOWN_OBJECTS = {
    "pushable", "unreachable", "collision", "timeout",
...
def _validate_params(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    obj = params.get("object", "pushable")
...
class PushZenohBridge:
    ...
    def __init__(self, settings: BridgeSettings | None = None):
        try:
            ...
            raise RuntimeError("Install eclipse-zenoh to run the push bridge.") from error
    ...
    def _on_action(self, sample) -> None:
        ...
        try:
            res = self._executor.execute("push_box", params)
    ...
    def spin(self) -> None:
        LOGGER.info("push-arm-001 bridge %s listening on %s", self.robot_id, self.action_topic)
        try:
            while True:
                time.sleep(0.1)
        finally:
