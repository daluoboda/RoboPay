...
+ facilitator) and the actual ``bridge.PushZenohBridge`` (real MuJoCo physics)
on a single Zenoh network, and asserts:

  * a paid ``push_box`` whose real MuJoCo run SUCCEEDS (object="pushable")
  ...
  * a paid ``push_box`` whose real MuJoCo run FAILS (object="unreachable")
    ...
    def _run_once(self, object_name: str):
        ...
        try:
            ...
            status, headers, _ = http_post(
                action_url, {"action": "push_box", "params": {"object": object_name}}
            )
