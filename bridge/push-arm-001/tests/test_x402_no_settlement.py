...
ROBOT_ID = "push-arm-001-no-settle"
...
class NoSettlementTests(unittest.TestCase):
    def _start(self, with_simulator: bool = True):
        ...
        simulator = InjectedPushSimulator(robot_id=ROBOT_ID) if with_simulator else None
        tunnel = None
        proxy.start()
        try:
    ...
    def _challenge(self, action_url: str) -> dict:
        status, headers, _ = http_post(action_url, {"action": "push_box", "params": {"object": "pushable"}})
