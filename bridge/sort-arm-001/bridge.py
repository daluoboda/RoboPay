"""Zenoh bridge for paid sort-arm-001 ``sort_object`` actions.
...
see already-paid actions, runs the real MuJoCo physics for ``sort_object``, and
...
LOGGER = logging.getLogger("robopay.sort")

ACTION_TOPIC = "robot/tunnel/action"
RESULT_TOPIC = "robot/tunnel/result"
METRICS_TOPIC = "robot/sort-arm-001/metrics"

ROBOT_ID = "sort-arm-001"
ALLOWED_ACTIONS = {"sort_object"}
PROFILE_ID = "sort.sort-arm-001.mujoco-sim.v1"

KNOWN_OBJECTS = {
    "sortable", "unreachable", "collision", "timeout",
...
def _validate_params(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    obj = params.get("object", "sortable")
...
class SortZenohBridge:
    ...
    def __init__(self, settings: BridgeSettings | None = None):
        try:
            ...
            raise RuntimeError("Install eclipse-zenoh to run the sort bridge.") from error
    ...
    def _on_action(self, sample) -> None:
        ...
        try:
            res = self._executor.execute("sort_object", params)
    ...
    def spin(self) -> None:
        LOGGER.info("sort-arm-001 bridge %s listening on %s", self.robot_id, self.action_topic)
        try:
            while True:
                time.sleep(0.1)
        finally:
