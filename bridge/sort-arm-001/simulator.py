"""sort-arm-001 --- MuJoCo backend for the RoboPay Tier 1 skill bridge.

Aligned with fabric-arm-001 / stack-arm-001 so all four arms are a sim-to-sim
comparison, not four demos:

  * The PHYSICS is real. Gravity, collision geometry, friction, contact forces
    and free-body dynamics of the sorted box are all solved by MuJoCo.
  * The CONTROLLER is deterministic. Fixed 6-stage plan (HOME -> MOVE_ABOVE ->
    DESCEND -> GRASP -> LIFT -> CARRY -> PLACE -> RELEASE -> SETTLE) from the
    shared closed-form keyframes. No runtime IK, so runs only fail for the
    bounty-relevant reasons.
  * The FAILURES are real. `unreachable` stretches and measures the shortfall;
    `collision` aborts on a genuine contact; `timeout` exhausts the budget;
    `misplace` means the box ended outside the target bin after release.

Public surface consumed by flow/executor.py:
    MuJoCoSimulator().sort_object(params) -> PickResult(success, reason, metrics)
"""
...
SKILL_ID = "sort_object"

# Floor-level target bin, offset from the pickup point but within arm reach
# at grasp height (max solvable r at z=GRASP_WZ is ~0.417 m).
TARGET_BIN_XY = (0.32, 0.24)
BIN_TOL = 0.05           # success tolerance on final box centre xy
STAGE_STEPS = {
...
# ----------------------------------------------------------------- scene table
SCENES = {
    "sortable":   {"box": (0.35, 0.0), "obstacle": None, "budget": DEFAULT_BUDGET},
...
def resolve_scene(params: dict | None):
    params = params or {}
    name = str(params.get("object", "sortable"))
...
_bx, _by = TARGET_BIN_XY
_bin_r = float(np.hypot(_bx, _by))
KEYFRAMES = {
    "home": solve(0.20, 0.42),
    "above": solve(WORK_R, GRASP_WZ + 0.14),
    "grasp": solve(WORK_R, GRASP_WZ),
    "lift": solve(WORK_R, GRASP_WZ + 0.15),
    "bin_above": solve(_bin_r, GRASP_WZ + 0.14),
    "bin_down": solve(_bin_r, GRASP_WZ),
...
class MuJoCoSimulator:
    ...
    def __init__(self):
        ...
        self.data = None
        self._steps = 0
        self._budget = SCENES["sortable"]["budget"]
    ...
    # ---------------------------------------------------------------- skill
    def sort_object(self, params: dict | None = None) -> PickResult:
        ...
        try:
            ...
            stage = "carry"
            if not self._run(KEYFRAMES["bin_above"], STAGE_STEPS["carry"], FINGER_CLOSED):
                return report(False, "collision", "obstacle struck during carry")
            stage = "place"
            if not self._run(KEYFRAMES["bin_down"], STAGE_STEPS["place"], FINGER_CLOSED):
        ...
        dx = end[0] - TARGET_BIN_XY[0]
        dy = end[1] - TARGET_BIN_XY[1]
        if end[2] > CUBE_HALF + 0.02:
            grasp_state = "dropped"
            return report(False, "misplace", f"box not on floor (z={end[2]:.3f})")
        if abs(dx) > BIN_TOL or abs(dy) > BIN_TOL:
            grasp_state = "misplaced"
            return report(False, "misplace",
                          f"box off bin by ({dx:+.3f},{dy:+.3f}) m")
        return report(True, "sorted", f"box sorted into bin at ({end[0]:.3f},{end[1]:.3f})")
