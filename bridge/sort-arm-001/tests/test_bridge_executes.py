...
+ facilitator) and the actual ``bridge.SortZenohBridge`` (real MuJoCo physics)
on a single Zenoh network, and asserts:

  * a paid ``sort_object`` whose real MuJoCo run SUCCEEDS (object="sortable")
  ...
  * a paid ``sort_object`` whose real MuJoCo run FAILS (object="unreachable")
    ...
    def _run_once(self, object_name: str):
        ...
        try:
            ...
            status, headers, _ = http_post(
                action_url, {"action": "sort_object", "params": {"object": object_name}}
            )
