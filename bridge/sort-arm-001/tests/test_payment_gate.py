    ...
    def test_unpaid_and_invalid_payment_fail_closed(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="fabric_payment_gate_") as temp_dir:
                ...
                # Prove the 402 challenge is wired for sort-arm-001.
                required = json.loads(
                ...
                malformed_status, _, _ = http_post(
                    action_url, {"action": "sort_object", "params": "not-an-object"}
                )
                ...
                tampered_id = f"fabric-tampered-payment-{uuid.uuid4().hex}"
