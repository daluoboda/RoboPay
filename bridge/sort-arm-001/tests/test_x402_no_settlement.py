...
ROBOT_ID = "sort-arm-001-no-settle"
...
class NoSettlementTests(unittest.TestCase):
    def _start(self, with_simulator: bool = True):
        ...
        simulator = InjectedSortSimulator(robot_id=ROBOT_ID) if with_simulator else None
        tunnel = None
        proxy.start()
        try:
    ...
    def _challenge(self, action_url: str) -> dict:
        status, headers, _ = http_post(action_url, {"action": "sort_object", "params": {"object": "sortable"}})
