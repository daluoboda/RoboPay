"""Zenoh bridge for paid stack-arm-001 ``stack_box`` actions.
...
see already-paid actions, runs the real MuJoCo physics for ``stack_box``, and
publishes the terminal result -- echoing the exact correlation tuple the
...
This mirrors the fabric-arm-001 fix for the reviewer's CHANGES_REQUESTED note:
settlement now flows through the real Tunnel + x402 facilitator, and the local
ledger in ``flow/payment.py`` is an in-process audit log only.
"""
...
LOGGER = logging.getLogger("robopay.stack")

ACTION_TOPIC = "robot/tunnel/action"
RESULT_TOPIC = "robot/tunnel/result"
METRICS_TOPIC = "robot/stack-arm-001/metrics"

ROBOT_ID = "stack-arm-001"
ALLOWED_ACTIONS = {"stack_box"}
PROFILE_ID = "stack.stack-arm-001.mujoco-sim.v1"
...
KNOWN_OBJECTS = {
    "stackable", "unreachable", "collision", "timeout",
...
def _validate_params(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    obj = params.get("object", "stackable")
...
class StackZenohBridge:
    ...
    def __init__(self, settings: BridgeSettings | None = None):
        try:
            ...
            raise RuntimeError("Install eclipse-zenoh to run the stack bridge.") from error
    ...
    def _on_action(self, sample) -> None:
        ...
        try:
            res = self._executor.execute("stack_box", params)
    ...
    def spin(self) -> None:  # pragma: no cover - integration entry point
        LOGGER.info(
            "stack-arm-001 bridge %s listening on %s; publishing results on %s",
...
def main() -> None:  # pragma: no cover - integration entry point
    """Run the stack-arm-001 bridge as a standalone Zenoh worker."""
    logging.basicConfig(level=logging.INFO)
    StackZenohBridge().spin()
