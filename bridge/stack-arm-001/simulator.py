...
Design contract (deliberately narrow, reviewer-first; aligned with
fabric-arm-001 so the two arms are a sim-to-sim comparison, not two demos):

  * The PHYSICS is real. Gravity, collision geometry, friction, contact forces
    and free-body dynamics of the manipulated box are all solved by MuJoCo.
    Nothing about the box is scripted.

  * The CONTROLLER is deterministic. The arm follows a fixed 6-stage plan
    (HOME -> MOVE_ABOVE -> DESCEND -> GRASP -> LIFT -> PLACE -> RELEASE ->
    SETTLE), built from the same closed-form 2-link keyframes used by
    fabric-arm-001. There is no runtime inverse kinematics, so a skill run can
    never fail for numerical reasons -- it only fails for the reasons the
    bounty cares about.

  * The FAILURES are real. `unreachable` drives the arm to full stretch and
    measures the residual tip-to-box distance. `collision` puts a rigid
    obstacle in the path and aborts on a genuine MuJoCo contact. `timeout`
    exhausts a hard step budget. `misstack` means the box ended up off the
    platform top or toppled after release -- no force on the platform, no
    success, and therefore no settlement upstream.

Public surface consumed by flow/executor.py:
    MuJoCoSimulator().stack_box(params) -> PickResult(success, reason, metrics)
"""
...
PLATFORM_H = 0.12        # fixed platform top height (m)
STACK_TOL = 0.012        # success tolerance on final box centre z
STAGE_STEPS = {
    "move_above": 70, "descend": 50, "grip": 60,
    "lift": 60, "place": 60, "settle": 30,
}
NOMINAL_STEPS = sum(STAGE_STEPS.values())

# The platform sits directly above the initial box xy, so a lifted box drops
# straight onto it.
PLATFORM_XY = (0.35, 0.0)
...
SCENES = {
    "stackable":   {"box": (0.35, 0.0), "obstacle": None, "budget": DEFAULT_BUDGET},
...
def resolve_scene(params: dict | None):
    ...
    params = params or {}
    name = str(params.get("object", "stackable"))
...
KEYFRAMES = {
    ...
    # place the box on the platform top (box centre == platform top + CUBE_HALF)
    "place": solve(WORK_R, PLATFORM_H + CUBE_HALF + GRIP_MID),
...
# ------------------------------------------------------------------- model --
def _model_xml(box_xy, platform_xy, obstacle_xy) -> str:
    """MJCF for the stacking cell.

    Collision bitmasks keep the scene honest:
        1 floor   2 box   4 finger pads   8 obstacle   16 arm links   32 platform
    box<->floor, box<->pads, box<->obstacle, box<->platform and
    arm<->obstacle, pads<->obstacle are live; arm<->box and arm<->floor are
    muted (shrouded manipulator), the standard way to model a gripper without
    inflating the contact set.
    """
    cx, cy = box_xy
    px, py = platform_xy
    obstacle = ""
...
<mujoco model="stack-arm-001">
  ...
  <worldbody>
    ...
    <!-- fixed stacking platform; top surface at z = PLATFORM_H -->
    <body name="platform" pos="{px} {py} {PLATFORM_H / 2}">
      <geom name="platform_g" type="box" size="0.06 0.06 {PLATFORM_H / 2}"
            rgba="0.35 0.45 0.55 1" contype="32" conaffinity="2"
...
class MuJoCoSimulator:
    """One instance == one robot. `stack_box` rebuilds the cell per call so
    ...
    def __init__(self):
        ...
        self.data = None
        self._steps = 0
        self._budget = SCENES["stackable"]["budget"]

    # ---------------------------------------------------------- scene setup
    def _build(self, scene: dict):
        xml = _model_xml(scene["box"], PLATFORM_XY, scene["obstacle"])
        ...
        self._plat_geom = gid("platform_g")
    ...
    def _apply(self, pose: dict, grip: float):
        """Pin the arm onto the commanded trajectory point (scripted chain;
        the box stays fully dynamic, so contacts/friction/gravity are real)."""
        d = self.data
    ...
    def _box_quat(self):
        return [float(v) for v in self.data.xquat[self._box_body]]
    ...
    # ---------------------------------------------------------------- skill
    def stack_box(self, params: dict | None = None) -> PickResult:
        ...
        try:
            ...
            # -- stage 3/6 GRIP (contact-gated closure)
            stage = "grip"
            n = STAGE_STEPS["grip"]
            for i in range(1, n + 1):
            ...
            # -- stage 4/6 LIFT (carry the box off the floor)
            stage = "lift"
            if not self._run(KEYFRAMES["lift"], STAGE_STEPS["lift"], FINGER_CLOSED):
                return report(False, "collision", "obstacle struck during lift")

            # -- stage 5/6 PLACE (lower onto the platform top)
            stage = "place"
            if not self._run(KEYFRAMES["place"], STAGE_STEPS["place"], FINGER_CLOSED):
                return report(False, "collision", "obstacle struck during place")

            # -- stage 6 RELEASE: detach + open grip, box rests on platform
            stage = "release"
            self._detach()
            self._run(dict(self._pose), 8, FINGER_OPEN)

            # -- SETTLE: prove the stack survives, not just the placement
            stage = "settle"
        ...
        final_z = self._box_pos()[2]
        target_z = PLATFORM_H + CUBE_HALF
        quat = self._box_quat()
        # unit quaternion is (1,0,0,0); tilt == departure from it
        tilt = abs(quat[0] - 1.0) + abs(quat[1]) + abs(quat[2]) + abs(quat[3])
        if final_z < target_z - STACK_TOL:
            grasp_state = "dropped"
            return report(False, "misstack",
                          f"box top z={final_z:.3f} < {target_z:.3f} (platform top)")
        if tilt > 0.05:
            grasp_state = "toppled"
            return report(False, "misstack", f"box tilted (quat offset {tilt:.3f})")
        return report(True, "stacked", f"box stacked at z={final_z:.3f} on platform")
