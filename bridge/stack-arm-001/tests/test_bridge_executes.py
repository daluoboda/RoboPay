...
+ facilitator) and the actual ``bridge.StackZenohBridge`` (real MuJoCo physics)
on a single Zenoh network, and asserts:

  * a paid ``stack_box`` whose real MuJoCo run SUCCEEDS (object="stackable")
  ...
  * a paid ``stack_box`` whose real MuJoCo run FAILS (object="unreachable")
    ...
    def _run_once(self, object_name: str):
        ...
        try:
            tunnel = launch_tunnel(
                ...
                Path(tempfile.mkdtemp(prefix="fabric_e2e_")),
            ...
            status, headers, _ = http_post(
                action_url, {"action": "stack_box", "params": {"object": object_name}}
            )
