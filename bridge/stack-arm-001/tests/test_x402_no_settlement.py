...
ROBOT_ID = "stack-arm-001-no-settle"
...
class NoSettlementTests(unittest.TestCase):
    def _start(self, with_simulator: bool = True):
        ...
        simulator = InjectedStackSimulator(robot_id=ROBOT_ID) if with_simulator else None
        tunnel = None
        proxy.start()
        try:
    ...
    def _challenge(self, action_url: str) -> dict:
        status, headers, _ = http_post(action_url, {"action": "stack_box", "params": {"object": "stackable"}})
