          ...
          <body name="elbow" pos="{LINK1} 0 0">
            ...
            <body name="wrist" pos="{LINK2} 0 0">
              ...
              <body name="cam_mount" pos="0 0 -{LINK3}">
                ...
                <site name="cam_site" pos="0 0 -{CAM_Z_OFFSET}" size="0.003"
                      rgba="0.9 0.3 0.3 0.6"/>
                <site name="cam_axis" pos="0 0 -{CAM_Z_OFFSET * 2}" size="0.002"
...
class MuJoCoSimulator:
    ...
    def _camera_axis(self) -> np.ndarray:
        """Direction the camera is pointing (from cam_site to cam_axis_site)."""
        pos = np.array(self.data.site_xpos[self._cam_site], dtype=float)
        axis = np.array(self.data.site_xpos[self._cam_axis_site], dtype=float)
        return axis - pos
    ...
    def active_inspection(self, params: dict | None = None) -> InspectionResult:
        ...
        return InspectionResult(all_confirmed, reason, build_metrics(
            engine=ENGINE, target=",".join(targets) if targets else "none",
