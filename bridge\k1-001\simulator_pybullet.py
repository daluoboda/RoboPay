...
{_joint("wrist_roll", "revolute", "wrist", "cam_mount", "0 0 -{LINK3}", "1 0 0", -3.14, 3.14)}
{_joint("cam_pan", "revolute", "cam_mount", "cam_mount", "0 0 0", "0 0 1", -1.57, 1.57)}
</robot>
"""
...
class PyBulletSimulator:
    ...
    def _camera_pos(self) -> list:
        # Use wrist link position as proxy
        link_idx = self._jidx.get("cam_mount", -1)
        if link_idx < 0:
            return [0.0, 0.0, 0.0]
    ...
    def active_inspection(self, params: dict | None = None) -> InspectionResult:
        ...
        return InspectionResult(all_confirmed, "all_targets_confirmed" if all_confirmed else "partial",
                                build_metrics(
                                    engine=ENGINE, target=",".join(targets),
