...
class TestRobotProfileMatchesSpec(unittest.TestCase):
    ...
    def test_scene_parameters_match_arm_spec(self):
        ...
        # Target positions should match arm_spec.TARGET_SPACING
        self.assertAlmostEqual(scene["targets"][0][1], -0.25, places=4)
        self.assertAlmostEqual(scene["targets"][2][1], 0.25, places=4)
        ...
        for root, dirs, files in os.walk(bridge_dir):
            ...
            for fname in files:
                ...
                for line in text.splitlines():
                    ...
                    if any(kw in stripped for kw in ("0x036CbD53842c5426634e7929541eC2318f3dCF7e", "0x036c")):
                        continue
                    self.assertNotRegex(
                        stripped,
                        r"0x[0-9a-fA-F]{64}",
