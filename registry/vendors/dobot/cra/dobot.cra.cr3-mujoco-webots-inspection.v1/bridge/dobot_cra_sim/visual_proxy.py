"""Small local HTTP/WebSocket proxy used only for operator-recorded E2E runs.

It forwards HTTP requests to a real Go Tunnel over the same WebSocket envelope
the hosted gateway uses.  It does not emulate x402, Zenoh, the simulator, or a
facilitator.  The live runner points Tunnel at the public facilitator.
"""
...
class TunnelConnection:
    ...
    def request(self, envelope: dict[str, Any], timeout: float = 45.0) -> dict[str, Any]:
        with self.lock:
            _write_frame(self.sock, json.dumps(envelope, separators=(",", ":")).encode("utf-8"))
...
class _Handler(http.server.BaseHTTPRequestHandler):
    ...
    def log_message(self, format: str, *args: Any) -> None:
        self.proxy.log(f"[proxy] {format % args}")
...
class LocalTunnelProxy:
    """Windows-hosted visual proxy for a Tunnel running on WSL or locally."""
