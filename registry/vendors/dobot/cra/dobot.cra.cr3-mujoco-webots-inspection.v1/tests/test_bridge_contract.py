...
from dobot_cra_sim.bridge import DobotCR3Execution, DurableReplayStore
...
def test_real_mujoco_action_is_durable_and_cannot_replay_after_restart(tmp_path: Path) -> None:
    ...
    first_boundary = DobotCR3Execution(replay_store=DurableReplayStore(state_path), episode_runner=run_mujoco_episode)
    first = first_boundary.process(event)
    assert first is not None
    assert first["status"] == "success"
    assert first["result"]["simulator_engine"] == "MuJoCo"
    assert len(first["result"]["observed_tags"]) == 3
    ...
    restarted_boundary = DobotCR3Execution(
...
def test_payment_fingerprint_blocks_same_payment_with_a_fresh_idempotency_key(tmp_path: Path) -> None:
    ...
    first = DobotCR3Execution(replay_store=store, episode_runner=run_mujoco_episode)
    ...
    second = DobotCR3Execution(replay_store=store, episode_runner=_runner_must_not_run)
