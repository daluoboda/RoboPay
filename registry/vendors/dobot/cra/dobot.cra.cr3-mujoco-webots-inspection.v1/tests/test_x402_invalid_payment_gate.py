            ...
            json.dumps(
                {
                    ...
                    "status": "failure",
                    "result": {"error_code": "INJECTED_DOBOT_SIMULATOR_FAILURE"},
                },
                separators=(",", ":"),
            ).encode("utf-8")
        )
...
def _tunnel_binary() -> Path:
    ...
    candidates.extend([REPO_ROOT / "bin" / "tunnel", REPO_ROOT / "bin" / "tunnel.exe"])
...
class _TunnelRun:
    def __init__(self, *, verify_response: dict, simulator_mode: str):
        ...
        self.tempdir = tempfile.TemporaryDirectory(prefix="dobot_x402_gate_")
        ...
        environment.update(
            {
                ...
                "ALLOWED_ACTIONS": f"{INSPECTION_SKILL},stop",
...
def test_facilitator_rejected_paid_shape_fails_before_action_boundary() -> None:
    ...
    try:
        run.start()
        body = {"action": INSPECTION_SKILL, "robot_id": run.robot_id, "params": {}}
...
def test_failed_or_timed_out_action_never_settles_and_replay_never_reexecutes(mode: str, expected_state: str) -> None:
    ...
    try:
        run.start()
        unpaid = requests.post(run.action_url, json={"action": INSPECTION_SKILL}, timeout=20)
        assert unpaid.status_code == 402
        action_id = f"dobot-no-settle-{mode}-{uuid.uuid4().hex}"
        body = {
