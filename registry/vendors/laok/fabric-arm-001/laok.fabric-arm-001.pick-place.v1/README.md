# laok × fabric-arm-001 — Tier 1 simulated pick-and-place

A Tier 1 RoboPay integration: a **simulated 2-DOF arm (`fabric-arm-001`)** that
performs a physics-executed pick-and-place skill in **MuJoCo**, gated by a
...
```
laok.fabric-arm-001.pick-place.v1/
├── robot.profile.yaml          # identity, runtime, submission scope/tier
├── skills.yaml                 # published skill: fabric_arm_pick_place
...
├── bridge/
│   ├── laok_fabric_arm_zenoh_bridge.py   # HTTP + Zenoh bridge
│   ├── x402_client.py                    # payment gate + facilitator verify/settle
│   ├── simulator.py                      # MuJoCo pick_object
│   ├── arm_spec.py
│   ├── requirements.txt
│   ├── config.example.json
...
export ROBOT_ID=fabric-arm-001-demo-001
...
python laok_fabric_arm_zenoh_bridge.py --port 8080
```
...
- **Real physics execution.** MuJoCo `pick_object` lifted the object `0.1313 m`
