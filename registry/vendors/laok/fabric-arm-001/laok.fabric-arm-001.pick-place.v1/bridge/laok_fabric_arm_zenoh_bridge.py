"""laok fabric-arm-001 RoboPay bridge (Tier 1 simulated manipulator).
...
4. The bridge validates the envelope (paramsHash, payment gate, idempotency),
   runs the MuJoCo pick_object skill, and publishes the correlated result to
   `robot/tunnel/result`.
...
SKILL_ID = "fabric_arm_pick_place"
...
class Bridge:
    ...
    def _execute(self, envelope: dict, key: str, auth: str | None, fp: str):
        ...
        try:
            ...
            res = self.sim.pick_object(envelope.get("params", {}) or {})
...
def main():
    ...
    ap.add_argument("--robot-id", default=os.environ.get("ROBOT_ID", "fabric-arm-001-demo-001"))
