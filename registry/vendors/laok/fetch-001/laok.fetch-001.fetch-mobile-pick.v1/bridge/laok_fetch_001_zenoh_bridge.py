"""laok fetch-001 RoboPay bridge (Tier 1 simulated manipulator).
...
4. The bridge validates the envelope (paramsHash, payment gate, idempotency),
   runs the MuJoCo fetch_mobile_pick skill, and publishes the correlated result to
   `robot/tunnel/result`.
...
class Bridge:
    ...
    def _execute(self, envelope: dict, key: str, auth: str | None, fp: str):
        ...
        try:
            ...
            res = self.sim.fetch_mobile_pick(envelope.get("params", {}) or {})
...
def main():
    ...
    ap.add_argument("--robot-id", default=os.environ.get("ROBOT_ID", "fetch-001-demo-001"))
