...
The mobile pick skill is driven by a **stage policy** (`bridge/arm_spec.py`), not a fixed joint target: the controller sequences approach -> grip -> lift -> traverse -> place -> verify, re-commanding from measured cube/contact state at every boundary, and verifies stable placement from simulator state before success is reported.
