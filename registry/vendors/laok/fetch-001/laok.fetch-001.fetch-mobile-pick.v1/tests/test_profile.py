"""Structural and privacy validation for the laok fetch-001 profile."""
import os
...
PROFILE_ID = "laok.fetch-001.fetch-mobile-pick.v1"
...
def test_robot_profile_fields():
    ...
    assert rp["robotModel"] == "fetch-001"
...
def test_action_envelope_example():
    import json
    env = json.load(open(os.path.join(PROFILE, "examples", "action-envelope.fetch-mobile-pick.json")))
    assert env["skillId"] == "fetch_mobile_pick"
