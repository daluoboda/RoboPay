...
4. The bridge validates the envelope (paramsHash, payment gate, idempotency),
   runs the MuJoCo push_object skill, and publishes the correlated result to
   `robot/tunnel/result`.
...
class Bridge:
    ...
    def _execute(self, envelope: dict, key: str, auth: str | None, fp: str):
        ...
        try:
            ...
            res = self.sim.push_object(envelope.get("params", {}) or {})
