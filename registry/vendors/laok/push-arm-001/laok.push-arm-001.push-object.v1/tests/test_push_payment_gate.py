"""Exercise push-arm-001's x402 payment gate through the real Go Tunnel binary.
...
SKILL_CATALOG = ROOT / "registry/vendors/laok/push-arm-001/laok.push-arm-001.push-object.v1" / "skill-catalog.json"
BRIDGE_PYTHONPATH = str(ROOT / "registry/vendors/laok/push-arm-001/laok.push-arm-001.push-object.v1/bridge")
ROBOT_ID = "push_arm_001_payment_gate"
ZENOH_TEST_PORT = int(os.environ.get("PUSH_PAYMENT_GATE_ZENOH_PORT", "7447"))
PRICE = "0.10"
ALLOWED_ACTIONS = "push_arm_push_object,stop"
...
class SimulatorSide:
    ...
    def _on_action(self, sample) -> None:
        ...
        res = self.executor.push_object(params)
...
class PushPaymentGateTests(unittest.TestCase):
    ...
    def test_unpaid_malformed_and_facilitator_rejected_requests_fail_closed(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="push_payment_gate_") as temp_dir:
                ...
                unpaid_status, unpaid_headers, _ = http_post(
                    action_url, {"action": "push_arm_push_object", "robot_id": ROBOT_ID}
                )
                ...
                malformed_status, _, _ = http_post(
                    action_url,
                    {"action": "push_arm_push_object", "params": "not-an-object"},
                )
                ...
                tampered_id = f"push-tampered-{uuid.uuid4().hex}"
                ...
                print("[PUSH DISCOVERY] robot + skills + price: OK")
                print("[PUSH PAYMENT GATE] unpaid/malformed/isValid:false -> HTTP 402, zero ActionEvents")
        finally:
    ...
    def test_paid_action_publishes_and_settles(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="push_paid_") as temp_dir:
                temp_dir = Path(temp_dir)
                config_path, _ = self._write_configs(temp_dir)
                tunnel = self._start_tunnel(
                    tunnel_binary, config_path, temp_dir, proxy, facilitator
                ...
                paid_id = f"push-paid-{uuid.uuid4().hex}"
                ...
                print("[PUSH PAID] verified payment -> ActionEvent -> correlated result -> settle: OK")
        finally:
            self._teardown(proxy, facilitator, facilitator_thread, observer, simulator, tunnel)
    ...
    def test_failed_execution_does_not_settle(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="push_fail_") as temp_dir:
                temp_dir = Path(temp_dir)
                config_path, _ = self._write_configs(temp_dir)
                tunnel = self._start_tunnel(
                    tunnel_binary, config_path, temp_dir, proxy, facilitator
                ...
                failed_id = f"push-fail-{uuid.uuid4().hex}"
                paid_status, _, _ = self._paid_post(
                ...
                print("[PUSH FAILURE] failed execution -> no settlement: OK")
        finally:
            self._teardown(proxy, facilitator, facilitator_thread, observer, simulator, tunnel)
    ...
    def test_timeout_does_not_settle(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="push_timeout_") as temp_dir:
                temp_dir = Path(temp_dir)
                config_path, _ = self._write_configs(temp_dir)
                tunnel = self._start_tunnel(
                    tunnel_binary, config_path, temp_dir, proxy, facilitator
                ...
                timeout_id = f"push-timeout-{uuid.uuid4().hex}"
                paid_status, _, _ = self._paid_post(
                ...
                print("[PUSH TIMEOUT] no simulator result -> timeout -> no settlement: OK")
        finally:
            self._teardown(proxy, facilitator, facilitator_thread, observer, simulator, tunnel)
