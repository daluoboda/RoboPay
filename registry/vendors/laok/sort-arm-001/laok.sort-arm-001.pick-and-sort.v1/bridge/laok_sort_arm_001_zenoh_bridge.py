"""laok sort-arm-001 RoboPay bridge (Tier 1 simulated manipulator).
...
4. The bridge validates the envelope (paramsHash, payment gate, idempotency),
   runs the MuJoCo pick_and_sort skill, and publishes the correlated result to
   `robot/tunnel/result`.
...
SKILL_ID = "sort_arm_pick_and_sort"
...
class Bridge:
    ...
    def list_skills(self) -> dict:
        ...
        return {
            ...
            "skills": [
                {
                    "skillId": SKILL_ID,
                    "name": "Physics-executed pick-and-sort",
    ...
    def _execute(self, envelope: dict, key: str, auth: str | None, fp: str):
        ...
        try:
            ...
            res = self.sim.pick_and_sort(envelope.get("params", {}) or {})
...
def main():
    ...
    ap.add_argument("--robot-id", default=os.environ.get("ROBOT_ID", "sort-arm-001-demo-001"))
