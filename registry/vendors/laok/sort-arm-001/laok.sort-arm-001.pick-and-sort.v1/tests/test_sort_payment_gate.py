"""Exercise sort-arm-001's x402 payment gate through the real Go Tunnel binary.
...
SKILL_CATALOG = ROOT / "registry/vendors/laok/sort-arm-001/laok.sort-arm-001.pick-and-sort.v1" / "skill-catalog.json"
BRIDGE_PYTHONPATH = str(ROOT / "registry/vendors/laok/sort-arm-001/laok.sort-arm-001.pick-and-sort.v1/bridge")
ROBOT_ID = "sort_arm_001_payment_gate"
ZENOH_TEST_PORT = int(os.environ.get("SORT_PAYMENT_GATE_ZENOH_PORT", "7447"))
PRICE = "0.10"
ALLOWED_ACTIONS = "sort_arm_pick_and_sort,stop"
...
class SimulatorSide:
    ...
    def _on_action(self, sample) -> None:
        ...
        res = self.executor.pick_and_sort(params)
...
class SortPaymentGateTests(unittest.TestCase):
    ...
    def test_unpaid_malformed_and_facilitator_rejected_requests_fail_closed(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="sort_payment_gate_") as temp_dir:
                ...
                unpaid_status, unpaid_headers, _ = http_post(
                    action_url, {"action": "sort_arm_pick_and_sort", "robot_id": ROBOT_ID}
                )
                ...
                malformed_status, _, _ = http_post(
                    action_url,
                    {"action": "sort_arm_pick_and_sort", "params": "not-an-object"},
                )
                ...
                tampered_id = f"sort-tampered-{uuid.uuid4().hex}"
                ...
                print("[SORT DISCOVERY] robot + skills + price: OK")
                print("[SORT PAYMENT GATE] unpaid/malformed/isValid:false -> HTTP 402, zero ActionEvents")
        finally:
    ...
    def test_paid_action_publishes_and_settles(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="sort_paid_") as temp_dir:
                temp_dir = Path(temp_dir)
                config_path, _ = self._write_configs(temp_dir)
                tunnel = self._start_tunnel(
                    tunnel_binary, config_path, temp_dir, proxy, facilitator
                ...
                paid_id = f"sort-paid-{uuid.uuid4().hex}"
                ...
                print("[SORT PAID] verified payment -> ActionEvent -> correlated result -> settle: OK")
        finally:
            self._teardown(proxy, facilitator, facilitator_thread, observer, simulator, tunnel)
    ...
    def test_failed_execution_does_not_settle(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="sort_fail_") as temp_dir:
                temp_dir = Path(temp_dir)
                config_path, _ = self._write_configs(temp_dir)
                tunnel = self._start_tunnel(
                    tunnel_binary, config_path, temp_dir, proxy, facilitator
                ...
                failed_id = f"sort-fail-{uuid.uuid4().hex}"
                paid_status, _, _ = self._paid_post(
                ...
                print("[SORT FAILURE] failed execution -> no settlement: OK")
        finally:
            self._teardown(proxy, facilitator, facilitator_thread, observer, simulator, tunnel)
    ...
    def test_timeout_does_not_settle(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="sort_timeout_") as temp_dir:
                temp_dir = Path(temp_dir)
                config_path, _ = self._write_configs(temp_dir)
                tunnel = self._start_tunnel(
                    tunnel_binary, config_path, temp_dir, proxy, facilitator
                ...
                timeout_id = f"sort-timeout-{uuid.uuid4().hex}"
                paid_status, _, _ = self._paid_post(
                ...
                print("[SORT TIMEOUT] no simulator result -> timeout -> no settlement: OK")
        finally:
            self._teardown(proxy, facilitator, facilitator_thread, observer, simulator, tunnel)
