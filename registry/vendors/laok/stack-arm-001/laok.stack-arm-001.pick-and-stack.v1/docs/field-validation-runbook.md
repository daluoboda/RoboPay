# Field Validation Runbook - stack
...
cd RoboPay
git checkout feat/stack-arm-001-tier1
```
...
# List available skills (should show stack variant)
python3 -m bridge.stack-arm-001 --discover
```

Expected: skill `laok.stack-arm-001.pick-and-stack.v1` listed with price 0.1 USDC on Base Sepolia.
...
curl -X POST http://localhost:8080/api/action \
  ...
  -d '{"skillId": "laok.stack-arm-001.pick-and-stack.v1", "params": {}}'
```
...
```bash
# Run full test suite
pytest bridge/stack-arm-001/tests/ -v

# Or trigger CI workflow (requires maintainer approve on fork PR)
gh run watch --workflow=stack-arm-001-ci.yml
```
...
|----------|----------|----------|
| `validation-report.md` | `bridge/stack-arm-001/docs/` | All 7 criteria covered |
| `x402-evidence.json` | Root | Real tx hashes, block numbers |
| `task-traceability.md` | `bridge/stack-arm-001/docs/` | Test-to-criterion mapping |
| `field-validation-runbook.md` | `bridge/stack-arm-001/docs/` | This runbook |
| `test_safe_stop.py` | `bridge/stack-arm-001/tests/` | Criterion #5 verification |
| `test_payment_gate.py` | `bridge/stack-arm-001/tests/` | Criterion #1, #4 verification |
| `settle.png` | `bridge/stack-arm-001/docs/evidence/` | Terminal payer=0xf274 |
| `stack-arm-demo.mp4` | `bridge/stack-arm-001/docs/evidence/` | Video of successful action |
| `evidence-manifest.yaml` | `bridge/stack-arm-001/docs/evidence/` | SHA-256 of all evidence files |
