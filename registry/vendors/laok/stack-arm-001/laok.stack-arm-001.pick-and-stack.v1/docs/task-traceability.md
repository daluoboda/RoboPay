...
|---|-----------|--------|----------|
| 1 | x402 verification **fails closed** before action dispatch | PASS | `test_stack_payment_gate.py` |
| 2 | Verified actions **correlated** through simulator result path | PASS | `test_stack_flow.py` / `test_simulator.py` |
| 3 | Settlement occurs **only after** successful execution | PASS | `test_stack_profiles.py` / `test_bridge.py` |
| 4 | Failure / timeout / replay paths **do not settle** | PASS | `test_sim2sim.py` / `test_stack_payment_gate.py` |
| 5 | Bounded policy + interruptible execution + **safe stop** | PASS | `test_stack_safe_stop.py` |
| 6 | MuJoCo/PyBullet results covered by reproducible **current-head CI** | PASS | `stack-arm-001-ci.yml` |
...
|-----------|--------|-------------|
| `test_stack_payment_gate.py` | #1, #4 | 15 sub-tests: unpaid 402, expired, replay, invalid sig, insufficient funds, wrong payer/payee/amount, checksum, timeout, failure, replay no double settle, unauthorized skill, missing payment |
| `test_stack_safe_stop.py` | #5 | 6 sub-tests: interruptible execution, bounded time, safe state, no settlement on interrupt, timeout handler, log entry |
| `test_stack_flow.py` | #2 | Action dispatch, result correlation, actionId flow |
...
| `test_stack_profiles.py` | #3 | Settlement trigger on SUCCESS, no settlement on FAILURE |
| `test_bridge.py` | #3, #4 | Bridge validation, Zenoh message routing, settlement routing |
| `stack-arm-001-ci.yml` | #6 | Full CI pipeline: test + verify_settlement + sim2sim |
...
6. `settle.png` / `stack-arm-demo.mp4` show payer=0xf274 in terminal output
