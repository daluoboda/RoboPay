"""Exercise stack-arm-001's x402 payment gate through the real Go Tunnel binary.
...
SKILL_CATALOG = ROOT / "registry/vendors/laok/stack-arm-001/laok.stack-arm-001.pick-and-stack.v1" / "skill-catalog.json"
BRIDGE_PYTHONPATH = str(ROOT / "registry/vendors/laok/stack-arm-001/laok.stack-arm-001.pick-and-stack.v1/bridge")
ROBOT_ID = "stack_arm_001_payment_gate"
ZENOH_TEST_PORT = int(os.environ.get("STACK_PAYMENT_GATE_ZENOH_PORT", "7447"))
PRICE = "0.10"
ALLOWED_ACTIONS = "stack_arm_pick_and_stack,stop"
...
class SimulatorSide:
    ...
    def _on_action(self, sample) -> None:
        ...
        res = self.executor.pick_and_stack(params)
...
class StackPaymentGateTests(unittest.TestCase):
    ...
    def _paid_post(self, action_url, unpaid_headers, action_id, params):
        return http_post(
            action_url,
            {
                "action": "stack_arm_pick_and_stack",
                "robot_id": ROBOT_ID,
                "action_id": action_id,
    ...
    def test_unpaid_malformed_and_facilitator_rejected_requests_fail_closed(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="stack_payment_gate_") as temp_dir:
                ...
                unpaid_status, unpaid_headers, _ = http_post(
                    action_url, {"action": "stack_arm_pick_and_stack", "robot_id": ROBOT_ID}
                )
                ...
                malformed_status, _, _ = http_post(
                    action_url,
                    {"action": "stack_arm_pick_and_stack", "params": "not-an-object"},
                )
                ...
                tampered_id = f"stack-tampered-{uuid.uuid4().hex}"
                ...
                print("[STACK DISCOVERY] robot + skills + price: OK")
                print("[STACK PAYMENT GATE] unpaid/malformed/isValid:false -> HTTP 402, zero ActionEvents")
        finally:
    ...
    def test_paid_action_publishes_and_settles(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="stack_paid_") as temp_dir:
                temp_dir = Path(temp_dir)
                config_path, _ = self._write_configs(temp_dir)
                tunnel = self._start_tunnel(
                    tunnel_binary, config_path, temp_dir, proxy, facilitator
                ...
                paid_id = f"stack-paid-{uuid.uuid4().hex}"
                ...
                print("[STACK PAID] verified payment -> ActionEvent -> correlated result -> settle: OK")
        finally:
            self._teardown(proxy, facilitator, facilitator_thread, observer, simulator, tunnel)
    ...
    def test_failed_execution_does_not_settle(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="stack_fail_") as temp_dir:
                temp_dir = Path(temp_dir)
                config_path, _ = self._write_configs(temp_dir)
                tunnel = self._start_tunnel(
                    tunnel_binary, config_path, temp_dir, proxy, facilitator
                ...
                failed_id = f"stack-fail-{uuid.uuid4().hex}"
                paid_status, _, _ = self._paid_post(
                ...
                print("[STACK FAILURE] failed execution -> no settlement: OK")
        finally:
            self._teardown(proxy, facilitator, facilitator_thread, observer, simulator, tunnel)
    ...
    def test_timeout_does_not_settle(self) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="stack_timeout_") as temp_dir:
                temp_dir = Path(temp_dir)
                config_path, _ = self._write_configs(temp_dir)
                tunnel = self._start_tunnel(
                    tunnel_binary, config_path, temp_dir, proxy, facilitator
                ...
                timeout_id = f"stack-timeout-{uuid.uuid4().hex}"
                paid_status, _, _ = self._paid_post(
                ...
                print("[STACK TIMEOUT] no simulator result -> timeout -> no settlement: OK")
        finally:
            self._teardown(proxy, facilitator, facilitator_thread, observer, simulator, tunnel)
