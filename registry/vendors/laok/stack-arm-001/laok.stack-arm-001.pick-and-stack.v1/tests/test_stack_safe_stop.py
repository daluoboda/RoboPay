"""Safe-stop / bounded-policy tests for stack-arm-001 — REAL MuJoCo.
...
The same simulator the paid flow uses (MuJoCoSimulator.pick_and_stack) is
...
class TestSafeStopReal:
    def test_collision_aborts_and_never_settles(self):
        ...
        sim = MuJoCoSimulator()
        result = sim.pick_and_stack({"collision_key": "collision"} if False else {"object": "collision"} if False else {"scene": "collision"} if False else {"object": "collision"})
    ...
    def test_timeout_stops_on_budget(self):
        ...
        sim = MuJoCoSimulator()
        result = sim.pick_and_stack({"object": "timeout"})
    ...
    def test_unreachable_stops_short(self):
        ...
        sim = MuJoCoSimulator()
        result = sim.pick_and_stack({"object": "unreachable"})
    ...
    def test_normal_scene_completes_within_budget(self):
        ...
        sim = MuJoCoSimulator()
        result = sim.pick_and_stack({"object": "cube"})
