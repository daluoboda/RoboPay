"""Profile-scoped, fail-closed Zenoh bridge for LimX TRON 1 simulation.
...
METRICS_TOPIC = "robot/limx_tron1/metrics"
READY_TOPIC = "robot/limx_tron1/ready"
PROFILE_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_REPLAY_DB = PROFILE_ROOT / "artifacts" / "state" / "limx_tron1_replay.sqlite3"
...
class LimXTron1ZenohBridge:
    """Subscribe to the private Tunnel action topic and publish terminal data."""

    def __init__(self, settings: BridgeSettings | None = None, execution: LimXTron1Execution | None = None):
        self.settings = settings or BridgeSettings.from_env()
        self.execution = execution or LimXTron1Execution()
