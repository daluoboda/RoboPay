...
from limx_tron1_sim.bridge import DurableReplayStore, LimXTron1Execution
...
def test_real_mujoco_action_is_durable_and_cannot_replay_after_restart(tmp_path: Path) -> None:
    ...
    first_boundary = LimXTron1Execution(replay_store=DurableReplayStore(state_path), episode_runner=run_mujoco_episode)
    ...
    restarted_boundary = LimXTron1Execution(
...
def test_payment_fingerprint_blocks_same_payment_with_a_fresh_idempotency_key(tmp_path: Path) -> None:
    ...
    first = LimXTron1Execution(replay_store=store, episode_runner=run_mujoco_episode)
    ...
    second = LimXTron1Execution(replay_store=store, episode_runner=_runner_must_not_run)
