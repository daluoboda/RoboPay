...
from limx_tron1_sim.contracts import NAVIGATION_SKILL, ROBOT_ID
from limx_tron1_sim.visual_proxy import LocalTunnelProxy, TunnelConnection
...
def test_websocket_reader_reassembles_continuation_frames() -> None:
    ...
    try:
        writer.sendall(
            _frame(b'{"id":"tron1-paid",', opcode=1, final=False)
            + _frame(b'"status":202}', opcode=0, final=True)
        )
        opcode, payload = TunnelConnection(reader)._read_message()
        assert opcode == 1
        assert json.loads(payload) == {"id": "tron1-paid", "status": 202}
    finally:
        reader.close()
        writer.close()
            ...
            json.dumps(
                {
                    ...
                    "status": "failure",
                    "result": {"error_code": "INJECTED_TRON1_SIMULATOR_FAILURE"},
                },
                separators=(",", ":"),
            ).encode("utf-8")
        )
...
class _TunnelRun:
    def __init__(self, *, verify_response: dict, simulator_mode: str):
        ...
        self.tempdir = tempfile.TemporaryDirectory(prefix="tron1_x402_gate_")
...
def test_failed_or_timed_out_action_never_settles_and_replay_never_reexecutes(mode: str, expected_state: str) -> None:
    ...
    try:
        ...
        assert unpaid.status_code == 402
        action_id = f"tron1-no-settle-{mode}-{uuid.uuid4().hex}"
        body = {
