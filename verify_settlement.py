...
def audit_tx(url, tx_hash, payer, payee, usdc, amount):
    ...
    if not transfers:
        ...
        for src, dst, raw in transfers:
            if src == payer and dst == payee:
                matched = raw
                break
        if matched is None:
            src, dst, _ = transfers[0]
            problems.append(
                "USDC Transfer is %s -> %s, but the evidence file declares %s -> %s"
                % (src, dst, payer, payee)
            )
        else:
    ...
    if matched is not None:
        note = "%s USDC %s -> %s" % (Decimal(matched) / USDC_DECIMALS, payer, payee)
...
def check_evidence_file(evidence):
    ...
    return problems, payer, payee, usdc, amount
...
def main():
    ...
    setup, payer, payee, usdc, amount = check_evidence_file(evidence)
    if setup:
    ...
    for tx_hash in hashes:
        try:
            problems, note = audit_tx(url, tx_hash, payer, payee, usdc, amount)
